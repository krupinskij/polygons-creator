...
export default class app {
  ...
  public static context: CanvasRenderingContext2D = app.canvas.getContext(
    '2d',
  ...
  public static hContext: CanvasRenderingContext2D = app.hCanvas.getContext(
    '2d',
  ) as CanvasRenderingContext2D;

  public static addButton: HTMLButtonElement = <HTMLButtonElement>(
    document.getElementById('button-add')
  );
  public static clearButton: HTMLButtonElement = <HTMLButtonElement>(
    document.getElementById('button-clear')
  );

  public static polygonsDiv: HTMLDivElement = <HTMLDivElement>(
    document.getElementById('div-polygons')
  );
  ...
  private static endAdding(): void {
    ...
    app.currentPolygon.vertices[
      app.currentPolygon.vertices.length - 1
    ].edgeColor = Color.Black;
    app.currentPolygon.vertices[
      app.currentPolygon.vertices.length - 1
    ].nextVertex = app.currentPolygon.vertices[0];
    app.currentPolygon.vertices[0].prevVertex =
      app.currentPolygon.vertices[app.currentPolygon.vertices.length - 1];
  ...
  private static addVertex(event: MouseEvent): void {
    ...
    if (
      app.currentPolygon.vertices.length > 2 &&
      calcDistance(app.prevPoint, app.currentPolygon.vertices[0].position) <= 10
    ) {
      app.endAdding();
      return;
    }
    ...
    if (app.currentPolygon.vertices.length > 1) {
      ...
      app.currentPolygon.vertices[
        ...
        app.currentPolygon.vertices[app.currentPolygon.vertices.length - 2];
    }
  ...
  private static moveCursor(event: MouseEvent): void {
    ...
    if (app.currentPolygon.vertices.length)
      drawLine(
        app.currentPolygon.vertices[0].position,
        app.currPoint,
        Color.Red,
      );
    ...
    if (
      app.currentPolygon.vertices.length > 2 &&
      calcDistance(app.currPoint, app.currentPolygon.vertices[0].position) <= 15
    ) {
