...
const saveInput: HTMLInputElement = document.getElementById("save-name") as HTMLInputElement;
const saveBtn: HTMLButtonElement = document.getElementById("save-json") as HTMLButtonElement;
			...
			if (vertex.relation != Relation.None) {
				if (!set.has(vertex.relationId)) {
					...
					jPolygon.relations.push({
						...
						e2: polygon.vertices.findIndex(el => el === vertex.correspondingVertex),
...
loadBtn.addEventListener('click', () => {
	...
	reader.addEventListener("load",  () => { loadPolygonsFromJSON(reader.result as string); });
	
	if(fileInput.files !== null && fileInput.files[0]) reader.readAsText(fileInput.files[0]);
...
function loadPolygonsFromJSON(data: string) {
	...
	Jpolygons.forEach((Jpolygon: IPolygon) => {
		...
		polygon.vertices[polygon.vertices.length - 1].nextVertex = polygon.vertices[0];
		polygon.vertices[0].prevVertex = polygon.vertices[polygon.vertices.length - 1];
		...
		Jpolygon.relations.forEach(relation => {
			...
			polygon.vertices[relation.e1].correspondingVertex = polygon.vertices[relation.e2];
			polygon.vertices[relation.e2].correspondingVertex = polygon.vertices[relation.e1];

			polygon.vertices[relation.e1].relation = relation.type == 0 ? Relation.Equal : Relation.Parallel;
			polygon.vertices[relation.e2].relation = relation.type == 0 ? Relation.Equal : Relation.Parallel;
