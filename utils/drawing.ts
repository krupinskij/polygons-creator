...
export function multisampling() {
	...
	const imgData2 = app.hContext.getImageData(0, 0, app.hCanvas.width, app.hCanvas.height);
	const arr = new Uint8ClampedArray(app.canvas.width * app.canvas.height * 4);

	for (let i = 0, j = 0; j < imgData2.data.length / 4; i += 8, j += 4) {

		if (i % (app.hCanvas.width * 4) === 0 && (i / (app.hCanvas.width * 4)) % 2 === 1) i += app.hCanvas.width * 4;

		arr[j] = Math.floor((imgData2.data[i] + imgData2.data[i + 4] + imgData2.data[i + app.hCanvas.width * 4] + imgData2.data[i + app.hCanvas.width * 4 + 4]) / 4);
		arr[j + 1] = Math.floor((imgData2.data[i + 1] + imgData2.data[i + 5] + imgData2.data[i + app.hCanvas.width * 4 + 1] + imgData2.data[i + app.hCanvas.width * 4 + 5]) / 4);
		arr[j + 2] = Math.floor((imgData2.data[i + 2] + imgData2.data[i + 6] + imgData2.data[i + app.hCanvas.width * 4 + 2] + imgData2.data[i + app.hCanvas.width * 4 + 6]) / 4);
		arr[j + 3] = 255;
...
function drawEdgesMS() {
	...
	app.polygons.forEach(polygon => {
		for (let i = 0; i < polygon.vertices.length; i++) {
			drawLineMS(polygon.vertices[i].position, polygon.vertices[(i + 1) % polygon.vertices.length].position, polygon.vertices[i].edgeColor);
	...
	app.polygons.forEach(polygon => {
		for (let i = 0; i < polygon.vertices.length; i++) {
			if (polygon.vertices[i].relation !== Relation.None) {
				drawRelationMark(polygon.vertices[i].position, polygon.vertices[(i + 1) % polygon.vertices.length].position, polygon.vertices[i].relation, polygon.vertices[i].relation)
			}
		}
