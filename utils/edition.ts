...
const movePolygonBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-movePolygon')
);
const moveVertexBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-moveVertex')
);
const addVertexBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-addVertex')
);
const deleteVertexBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-deleteVertex')
);
const moveEdgeBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-moveEdge')
);
const addRelationBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-addRelation')
);
const deleteRelationBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-deleteRelation')
);

const equalRelationBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-relation-equal')
);
const parallelRelationBtn: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-relation-parallel')
);

const cancelButton: HTMLButtonElement = <HTMLButtonElement>(
  document.getElementById('button-polygon-cancel')
);
...
let moveVertexController = {
  ...
  mousemoveEventHandler: (event: MouseEvent) => {
    ...
    if (
      moveVertexController.vertexToMove &&
      !moveVertexController.canMoveVertex
    ) {
    ...
    for (const vertex of app.currentPolygon.vertices) {
      ...
      if (calcDistance(app.currPoint, vertex.position) < 10) {
        ...
        moveVertexController.indexToMove = app.currentPolygon.vertices.findIndex(
          (el) => el === vertex,
        );
    ...
    if (moveVertexController.canMoveVertex) {
      ...
      moveVertexController.vertexToMove.position.x +=
        app.currPoint.x - app.prevPoint.x;
      moveVertexController.vertexToMove.position.y +=
        app.currPoint.y - app.prevPoint.y;
      app.prevPoint = app.currPoint;
...
let addVertexController = {
  ...
  mousemoveEventHandler: (event: MouseEvent) => {
    ...
    for (const vertex of app.currentPolygon.vertices) {
      ...
      if (
        areInLine(vertex.position, vertex.nextVertex.position, app.currPoint)
      ) {
        vertex.color = Color.Red;
  ...
  mousedownEventHandler: (event: MouseEvent) => {
    ...
    if (addVertexController.canAddVertex) {
      ...
      const index = app.currentPolygon.vertices.findIndex(
        (elem) => elem === addVertexController.vertexToAdd,
      );
      app.currentPolygon.vertices.splice(index + 1, 0, new Vertex(currPoint));

      app.currentPolygon.vertices[index + 1].nextVertex =
        addVertexController.vertexToAdd.nextVertex;
      addVertexController.vertexToAdd.nextVertex =
        app.currentPolygon.vertices[index + 1];

      app.currentPolygon.vertices[index + 1].prevVertex =
        addVertexController.vertexToAdd;
      (app.currentPolygon.vertices[index + 1].nextVertex as Vertex).prevVertex =
        app.currentPolygon.vertices[index + 1];

      if (addVertexController.vertexToAdd.correspondingVertex !== null) {
        addVertexController.vertexToAdd.correspondingVertex.relation =
          Relation.None;
...
let moveEdgeController = {
  ...
  mousemoveEventHandler: (event: MouseEvent) => {
    ...
    if (
      moveEdgeController.edgeToMove.length === 2 &&
      !moveEdgeController.canMoveEdge
    ) {
    ...
    if (moveEdgeController.canMoveEdge) {
      ...
      app.prevPoint = app.currPoint;
      correctRelations(
        app.currentPolygon.vertices.findIndex(
          (el) => el === moveEdgeController.edgeToMove[0],
        ),
      );
      refreshCanvas();
    }
...
const makeParallelController = {
  ...
  mousemoveEventHandler: (event: MouseEvent) => {
    ...
    if (
      ...
      makeParallelController.relationVertex !==
        makeParallelController.relationVertices[0]
    ) {
      if (
        !makeParallelController.relationVertices[0] ||
        makeParallelController.relationVertex !==
          makeParallelController.relationVertices[0].nextVertex
      ) {
      ...
      if (
        !makeParallelController.relationVertices[0] ||
        makeParallelController.relationVertex.nextVertex !==
          makeParallelController.relationVertices[0]
      ) {
  ...
  mousedownEventHandler: (event: MouseEvent) => {
    if (makeParallelController.canAddRelation) {
      ...
      makeParallelController.relationVertices.push(
        makeParallelController.relationVertex as Vertex,
      );
      makeParallelController.relationVertex.relation = Relation.Parallel;

      if (makeParallelController.relationVertices.length === 2) {
        if (
          makeParallelController.relationVertices[0].nextVertex === null ||
          makeParallelController.relationVertices[1].nextVertex === null
        ) {
          throw new Error('Wystąpił błąd podczas dodawania relacji!');
        }
        ...
        addRelation(
          makeParallelController.relationVertices[0],
          makeParallelController.relationVertices[1],
        );
        ...
        makeParallelController.relationVertices[0].nextVertex.color =
          Color.Black;
        ...
        makeParallelController.relationVertices[1].nextVertex.color =
          Color.Black;
...
let deleteRelationController = {
  ...
  mousemoveEventHandler: (event: MouseEvent) => {
    ...
    if (
      ...
      !deleteRelationController.canDeleteRelation
    ) {
  ...
  mousedownEventHandler: (event: MouseEvent) => {
    if (deleteRelationController.canDeleteRelation) {
      ...
      deleteRelationController.vertexToDelete.correspondingVertex.relation =
        Relation.None;
  ...
  mouseupEventHandler: (event: MouseEvent) => {
    if (
      deleteRelationController.vertexToDelete === null ||
      deleteRelationController.vertexToDelete.nextVertex === null
    ) {
      throw new Error('Wystąpił błąd podczas usuwania relacji!');
    }
...
parallelRelationBtn.addEventListener('click', (event: MouseEvent) => {
  switchOn.call(makeParallelController);
});
equalRelationBtn.addEventListener('click', (event: MouseEvent) => {
  switchOn.call(makeEqualController);
});

deleteRelationBtn.addEventListener('click', (event: MouseEvent) => {
  switchOn.call(deleteRelationController);
});
...
function switchOn(this: IController) {
  if (app.currentPolygon === null)
    throw new Error('Wystąpił błąd podczas próby modyfikacji!');
