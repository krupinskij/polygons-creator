...
export function correctRelations(id: number) {
	...
	do {
		...
		if (vertex.relation === Relation.Equal) {
            ...
            if(vertex.nextVertex === null || correspondingVertex.nextVertex === null) {
                throw new Error("Wystąpił błąd podczas poprawiania relacji!");
            }

			const distance1 = calcDistance(vertex.position, vertex.nextVertex.position);
			const distance2 = calcDistance(correspondingVertex.position, correspondingVertex.nextVertex.position);
			const a = distance1 / distance2;

			correspondingVertex.position.x = Math.floor(correspondingVertex.nextVertex.position.x + (correspondingVertex.position.x - correspondingVertex.nextVertex.position.x) * a);
			correspondingVertex.position.y = Math.floor(correspondingVertex.nextVertex.position.y + (correspondingVertex.position.y - correspondingVertex.nextVertex.position.y) * a);
			...
			if (Math.abs(vertex.position.y - vertex.nextVertex.position.y) < Math.abs(vertex.position.x - vertex.nextVertex.position.x)) {
				...
				correspondingVertex.position.y = Math.floor(correspondingVertex.nextVertex.position.y - t * (correspondingVertex.nextVertex.position.x - correspondingVertex.position.x));
				...
				correspondingVertex.position.x = Math.floor(correspondingVertex.nextVertex.position.x - t * (correspondingVertex.nextVertex.position.y - correspondingVertex.position.y));
